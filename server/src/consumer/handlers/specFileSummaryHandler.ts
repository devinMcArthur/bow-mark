...
import type { SpecFileSummaryMessage } from "../../rabbitmq/publisher";
import { summarizePdf, DocumentSummary } from "./summarizePdf";

const anthropic = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });

const SUMMARY_PROMPT = `You are processing a construction specification or reference document for a paving/concrete company.
Analyze this document and return a JSON object with exactly these fields:
{
  ...
  "documentType": "what type of document this is (e.g. Municipal Spec, Standard Drawing, Material Standard, Design Guide, Code Reference, etc.)",
  "keyTopics": ["array", "of", "key", "topics", "materials", "or", "requirements", "mentioned"]
}
Return only valid JSON, no markdown, no explanation.`;

export const specFileSummaryHandler = {
  async handle(message: SpecFileSummaryMessage): Promise<void> {
    const { fileObjectId, fileId } = message;
    console.log(`[SpecSummary] Processing file ${fileId}`);

    await System.findOneAndUpdate(
      { "specFiles._id": fileObjectId },
      { $set: { "specFiles.$.summaryStatus": "processing" } }
    );
    ...
    try {
      ...
      await System.findOneAndUpdate(
        ...
        {
          $set: {
            "specFiles.$.summary": summary,
            "specFiles.$.summaryStatus": "ready",
            ...(pageCount !== undefined ? { "specFiles.$.pageCount": pageCount } : {}),
          },
        }
      );

      console.log(`[SpecSummary] Done for file ${fileId}`);
    } catch (error) {
      console.error(`[SpecSummary] Failed for file ${fileId}:`, error);
      const summaryError = error instanceof Error ? error.message : String(error);
      await System.findOneAndUpdate(
        { "specFiles._id": fileObjectId },
        { $set: { "specFiles.$.summaryStatus": "failed", "specFiles.$.summaryError": summaryError } }
      );
      throw error;
    }
  },
