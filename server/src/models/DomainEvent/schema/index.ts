...
export class DomainEventPatchOpClass {
  @Field() @prop({ required: true, enum: ["add", "remove", "replace"] })
  public op!: "add" | "remove" | "replace";

  @Field() @prop({ required: true })
  public path!: string;

  // `value` is free-form JSON. `Field(() => Object)` intentionally loose —
  // strict typing the value across every event type is not feasible.
...
export class DomainEventSchema {
  ...
  // identity
  @Field() @prop({ required: true, index: true })
  public type!: string;

  @Field() @prop({ required: true, default: 1 })
  public schemaVersion!: number;

  // actor
  @Field() @prop({ required: true, enum: ["user", "ai", "system"] })
  public actorKind!: "user" | "ai" | "system";

  @Field(() => ID, { nullable: true })
  @prop({ ref: () => UserClass, required: false })
  public actorId?: Ref<UserClass>;

  @Field(() => ID, { nullable: true })
  @prop({ ref: () => UserClass, required: false })
  public onBehalfOf?: Ref<UserClass>;
  ...
  // time
  @Field() @prop({ required: true, default: () => new Date() })
  public at!: Date;
