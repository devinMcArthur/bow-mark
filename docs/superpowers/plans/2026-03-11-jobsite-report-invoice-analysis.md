# Jobsite Report Invoice Analysis Implementation Plan

> **For agentic workers:** REQUIRED: Use superpowers:subagent-driven-development (if subagents available) or superpowers:executing-plans to implement this plan. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the minimal invoice tables in the Jobsite Report with a rich grouped/flat view showing date, description, type badge, and sortable columns.

**Architecture:** Extract a new `InvoiceTable` component that handles all rendering logic (grouped view, flat view, sorting). `ExpenseInvoices` and `RevenueInvoices` are simplified to just pass their invoice arrays into `InvoiceTable`. No server or GraphQL changes needed — `date` and `description` are already fetched.

**Tech Stack:** React, Chakra UI, TypeScript. No new dependencies.

**Design doc:** `docs/superpowers/specs/2026-03-11-jobsite-report-invoice-analysis.md`
...
| File | Change |
|---|---|
| `client/src/components/Common/JobsiteReport/InvoiceTable.tsx` | **CREATE** — new shared component, all grouped/flat/sort logic lives here |
| `client/src/components/Common/JobsiteReport/ExpenseInvoices.tsx` | **MODIFY** — replace inline table with `<InvoiceTable>` |
| `client/src/components/Common/JobsiteReport/RevenueInvoices.tsx` | **MODIFY** — replace inline table with `<InvoiceTable>` |
| `client/src/components/Common/JobsiteReport/InvoiceSummary.tsx` | **UNCHANGED** |

---

## Chunk 1: All Tasks

### Task 1: Create InvoiceTable.tsx

**Files:**
- Create: `client/src/components/Common/JobsiteReport/InvoiceTable.tsx`

The component accepts an array of `JobsiteDayReportInvoiceSnippetFragment` and renders either a grouped or flat table, controlled by internal state. No props needed beyond the data and a caption string.
...
```bash
cd /home/dev/work/bow-mark/client && npm run type-check 2>&1 | head -30
```
...
```bash
git add client/src/components/Common/JobsiteReport/InvoiceTable.tsx
git commit -m "feat(report): add InvoiceTable component with grouped and flat views"
```

---

### Task 2: Update ExpenseInvoices and RevenueInvoices to use InvoiceTable

**Files:**
- Modify: `client/src/components/Common/JobsiteReport/ExpenseInvoices.tsx`
- Modify: `client/src/components/Common/JobsiteReport/RevenueInvoices.tsx`

Replace the inline table in each with `<InvoiceTable>`. The Card heading collapse behaviour is preserved. `InvoiceSummary` stays exactly as-is.

- [ ] **Step 1: Update ExpenseInvoices.tsx**
...
- [ ] **Step 2: Update RevenueInvoices.tsx**
...
Note: Revenue invoices use `colorScheme="green"` to visually distinguish them from expense invoices.
...
```bash
git add client/src/components/Common/JobsiteReport/ExpenseInvoices.tsx \
        client/src/components/Common/JobsiteReport/RevenueInvoices.tsx
git commit -m "feat(report): wire InvoiceTable into expense and revenue invoice sections"
```
